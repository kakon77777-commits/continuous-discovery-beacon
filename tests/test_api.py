...
def test_create_dispatch_and_public_outputs(client, auth_headers):
    ...
    response = client.post("/api/v1/events", headers=auth_headers, json=payload)
    assert response.status_code == 201, response.text
    data = response.json()
...
def test_event_deduplication(client, auth_headers):
    ...
    first = client.post("/api/v1/events", headers=auth_headers, json=payload)
    second = client.post("/api/v1/events", headers=auth_headers, json=payload)
